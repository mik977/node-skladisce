...
var addr2daddr = exports.addr2daddr = function(addr) {
  return (((addr >> 24) & 0xff) + '.' + ((addr >> 16) & 0xff) + '.' + (addr & 0xffff));
};

var daddr2addr = exports.daddr2addr = function(daddr) {
  var parts = daddr.split('.');
  return (((parts[0] & 0xff) << 24) + ((parts[1] & 0xff) << 16) + (parts[2] & 0xffff));
};

var json2redis = exports.json2redis = function(obj) {
  var objx = {};
  for (var item in obj) {
    objx[item] = obj[item].toString();
  }
  return objx;
};
...
var auth = exports.auth = {
  ...
  basicAuth: function (request, body, callback) {
    ...
    if (scheme == 'Basic') {
      ...
      if (parts.length < 2) {
        return callback(new journey.NotAuthorized("Authorization scheme not in the form: ApiKey [apikey] [clientkey]"));
      }
      var apikey = parts[1]; // apikey
      if (apikey != auth.apiKey) {
        return callback(new journey.NotAuthorized("Wrong API key"));
      }
      return callback(null);
    } else {
